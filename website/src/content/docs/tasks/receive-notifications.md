...
See the [AgentDefinition CRD](/reference/crd-agentdefinition/) reference for the full `output`
schema, including `select` filters and `headers_secret` for authenticated listeners.

## Launch a run and watch the events

[Launch an agent](/tasks/launch-an-agent/) against a station using that recipe. As the run
...
the run. See [Delivery & retry](/reference/notification-api/#delivery--retry) to tune it.
