...
| `mcp` | `resources.mcp_servers` is non-empty and the model routes to a vendor whose CLI reads MCP servers from a settings file rather than a flag (today Gemini) | writes the servers to a file of the subsystem's own, and the controller points the CLI at it through the env. For Gemini that is `/agent/.ai-agent/gemini-settings.json`, `{"mcpServers": …}` with `httpUrl` for http, `url` for sse and `command` + `args` for stdio, named by `GEMINI_CLI_SYSTEM_SETTINGS_PATH`: gemini-cli loads it as its system scope and merges `mcpServers` by server name over the user scope, so servers a hook bundle declares in `.gemini/settings.json` still work and the bundle's file is never read or rewritten. The file sits outside `.gemini`, so a conversation snapshot neither carries it nor restores an old one, and it is written whole, so no server from a previous run survives. A `headers_secret` becomes an `Authorization` value of `${NAME}`, the same shell-safe variable Claude's `--mcp-config` references; the CLI expands it from the pod env when it loads the file, so the credential never reaches disk. Claude needs no step: its adapter passes the servers on the command line. A headless Gemini run can only call the tools under `permission_mode: bypass` (`--yolo`), since the Gemini adapter maps no allowed-tools list. Because the variable moves gemini-cli's system scope rather than adding to it, a Station image's own system settings (its default `/etc/gemini-cli/settings.json`) are not loaded on a Gemini run that declares MCP servers; its system-defaults file still is. A recipe env var or secret named like the vendor's variable is dropped on such a run, as a reserved name would be. A failed write fails the init. |
