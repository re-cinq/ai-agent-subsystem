...
| `mcp` | `resources.mcp_servers` is non-empty and the model routes to a vendor whose CLI reads MCP servers from a settings file rather than a flag (today Gemini) | merges the servers into that vendor's settings under `$HOME` — for Gemini, `mcpServers` in `.gemini/settings.json` (`httpUrl` for http, `url` for sse, `command` + `args` for stdio) — with `jq`, keeping whatever a hook bundle already wrote there. A `headers_secret` becomes an `Authorization` value of `${NAME}`, the same shell-safe variable Claude's `--mcp-config` references; the CLI expands it from the pod env when it loads the file, so the credential never reaches disk and never rides a conversation snapshot. Last, after `conversation`, so a restored state directory cannot overwrite it. Claude needs no step: its adapter passes the servers on the command line. A failed write fails the init — a declared server the agent silently lacks is the failure this exists to end. |
