...
- **`prompt`** — the task template. Supports `{placeholder}` tokens filled from an Agent's
  `parameters` at run time (see [Prompt templating](/ai-agent-subsystem/reference/prompt-templating/)).
...
across many [Stations](/ai-agent-subsystem/concepts/station/) — a dev kind cluster, a CI namespace,
or production — by pairing it with a different Pod template.

The full field reference is in
[AgentDefinition CRD](/ai-agent-subsystem/reference/crd-agentdefinition/).
